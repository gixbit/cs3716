...
public class Register extends JPanel implements PanelAccess {
	...
	// private ArrayList<PlayerPanel> listOfPlayers = new
	// ArrayList<PlayerPanel>();
	 ...
	 * tournamentBox){ String selected =
	 * tournamentBox.getSelectedItem().toString(); for(int i = 0; i <
	 * numOfTourns; i++){ if(selected.equals(listOfTourns.get(i).getName())){
	 * dateLabel.setText("Registration closes on " +
	 * listOfTourns.get(i).getEndDate()); revalidate(); break; } } } else
	 * if(event.getSource() == addButton){ if(n >= 40){ //JFrame frame1 = new
	 * popUp("You Have Reached the Max Number of Players.");
	 * //frame1.setVisible(true); //dispose(); } else{ n++; listOfPlayers[n-1] =
	 * new PlayerPanel(); playerPanel.add(listOfPlayers[n-1]);
	 * players.add(playerPanel); revalidate(); repaint(); } } else
	 * if(event.getSource() == cancelButton){ //JFrame frame1 = new
	 * MainScreen(listOfTourns); //frame1.setVisible(true); //dispose(); } else
	 * if(event.getSource() == clearButton){ //JFrame frame1 = new
	 * Register(listOfTourns); //frame1.setVisible(true); //dispose(); } else{
	 * //event.getSource() == RegisterButton int index = 0; for(int i=0; i <
	 * tournNames.size(); i++){ if(tournNames.get(i) ==
	 * (String)tournamentBox.getSelectedItem()){ //TODO: FIX ME JAIMEE
	 * listOfTourns.get(i).addTeam(new Team(teamNameField.getText(),
	 * coachField.getText())); index = i; break; } } //JFrame frame1 = new
	 * ListOfTeams(listOfTourns.get(index), listOfTourns);
	 * //frame1.setVisible(true); //dispose();
	 * System.out.println(organizNameField.getText()); for(int i=0; i < n; i++){
	 * if(listOfPlayers[i].getPlayerName() != null &&
	 * listOfPlayers[i].getPlayerAge() != null){
