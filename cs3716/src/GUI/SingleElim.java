...
public class SingleElim extends JPanel {
	...
	private ArrayList<JComponent> panList = new ArrayList<JComponent>(); // FIX
																			// ME
	private Tournament tournament; // FIX ME

	// numT needs to be replaced with an arrayList of teams and numOf divisions
	// also need to be an input

	/**
	 * Constructor for SingleElimination. <br>
	...
	private void createBorders() {
		...
		for (int j = 0; j < 2 * numOfTeams; j++) {
			...
			/*
			 * if((j-spacers)%height == 0){ top = 1; bottom = 0; side = 5; }
			 * else if((j-spacers)%height == height-1){ top = 0; bottom = 1;
			 * side = 5; } else{
			 */ top = 0;
			bottom = 0;
			side = 5;
		...
		/*
		 * if(numOfColns == colnNum){ line = BorderFactory.createMatteBorder(1,
		 * 0, 0, 0, Color.black);
		 * panList.get(2*numOfTeams-spacers).setBorder(line); } else{
		 */ line = BorderFactory.createMatteBorder(0, 0, 0, 0, Color.black);
